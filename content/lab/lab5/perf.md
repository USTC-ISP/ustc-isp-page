...
- 当 \(N=1024\) 时，`input` 矩阵一行有 1024 个 float（4KB），L1 Cache 装不下一整行，数据会频繁被替换。
...
- 每次只处理 \(BLOCK\_SIZE \times BLOCK\_SIZE\) 大小的子块（比如 \(32 \times 32 = 1024\) 个 float，仅 4KB），这个大小的块可以完整放在 L1 Cache 里。
